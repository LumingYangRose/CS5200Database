...
router.post("/homes/delete", async (req, res) => {
  try {
    ...
    if (changes === 0) {
      req.session.err = `Couldn't delete the object ${home.ID}`;
      res.redirect("/homes");
      return;
    }
...
// router.post("/homes/search", async (req, res) => {
//   const zip = req.body;

//   try {
//     console.log("Search neighborhood", zip);
//     const homes = await myDB.searchHome(zip, res);
//     req.session.msg = "Home found";
